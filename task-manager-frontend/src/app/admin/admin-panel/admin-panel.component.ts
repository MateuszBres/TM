...
export class AdminPanelComponent implements OnInit {
  ...
  users: userResponse[] = [];
  displayedColumns: string[] = ['id', 'email', 'role', 'createdAt', 'edit', 'delete'];
  ...
  editRole(user: userResponse) {
    ...
    this.service.updateUserRole(user.id, newRole).subscribe(() => {
      user.role = newRole;
      this.snack.success( `Rola użytkownika ${user.email} została zmieniona na ${newRole}`);
    });
  });
  }
